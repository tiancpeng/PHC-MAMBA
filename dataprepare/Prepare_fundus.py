...
train_number = 0 # Randomly assign the number of images for generating the training set.
val_number = 0  # Randomly assign the number of images for generating the validation set.
test_number = 0  # Randomly assign the number of images for generating the test set.
all = int(train_number) + int(val_number) + int(test_number)

############################################################# Prepare your data set #################################################
Tr_list = glob.glob("/public/home/xukai_tcp_2607/final-UltraLight-VM-UNet/dataprepare/DRIVE/images/"+'/*.png')   # Images storage folder. The image type should be 24-bit png format.
...
for idx in range(len(Tr_list)):
    ...
    b = b[len(b)-7: len(b)-4]
    print(b)
    add = ("/public/home/xukai_tcp_2607/final-UltraLight-VM-UNet/dataprepare/DRIVE/masks/" + b +'.png')  # Masks storage folder. The Mask type should be a black and white image of an 8-bit png (0 pixels for the background and 255 pixels for the target).
    img2 = sc.imread(add)
